...
void Shr3dder::process(std::map<ImageType,std::string> outputFilenames) {
    for (std::pair<ImageType,std::string> elem : outputFilenames) {
        ...
        if (!b)
            pdal::pdal_error("Failed to write" + elem.second);
    }
}
...
void Shr3dder::createDSM() {
    m_log->get(pdal::LogLevel::Debug) << "Creating DSM" << std::endl;
...
void Shr3dder::createMIN() {
    m_log->get(pdal::LogLevel::Debug) << "Creating MIN" << std::endl;
...
void Shr3dder::createDSM2() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Creating DSM2" << std::endl;
...
void Shr3dder::createDTM0() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Creating DTM0" << std::endl;
    ...
    m_log->get(pdal::LogLevel::Debug) << "Setting DTM0 values to VOID where DSM is VOID..." << std::endl;
    for (unsigned int j = 0; j < minImage.height; j++) {
        for (unsigned int i = 0; i < minImage.width; i++) {
...
void Shr3dder::createDTM() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Creating DTM"<< std::endl;
...
void Shr3dder::createIntensity() {
    const OrthoImage<unsigned short> &dsmImage = getDSM();
    m_log->get(pdal::LogLevel::Debug) << "Creating INTensity image"<< std::endl;
    if (is_pset_empty())
        throw pdal::pdal_error("Point cloud undefined / empty\n");
...
void Shr3dder::createMinAGL() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Creating MINAGL image"<< std::endl;
...
void Shr3dder::labelClasses() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Creating CLS image"<< std::endl;
...
void Shr3dder::labelBuildings() {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Labeling buildings" << std::endl;
...
void Shr3dder::createOutlines() {
    m_log->get(pdal::LogLevel::Debug) << "Creating building outlines" << std::endl;
    ...
    m_log->get(pdal::LogLevel::Debug) << "Traced " << bounds.size() << " building outlines." << std::endl;
    ...
    m_log->get(pdal::LogLevel::Debug) << "After simplifying, left with " << bldgOutlines.size() << " building outlines." << std::endl;
...
//     m_log->get(pdal::LogLevel::Debug) << "Max group size: " << maxGroupSize << std::endl;
//     m_log->get(pdal::LogLevel::Debug) << "Number of cropped groups: " << bigCount << std::endl;
}
...
void Shr3dder::classifyGround(OrthoImage<unsigned int> &labelImage, const OrthoImage<unsigned short> &dsmImage,
    ...
    // Fill voids.
    m_log->get(pdal::LogLevel::Debug) << "Filling voids" << std::endl;
    ...
    for (unsigned int k = 0; k < numIterations; k++) {
        m_log->get(pdal::LogLevel::Debug) << "Iteration# " << k+1 << std::endl;

        // Label the object boundaries.
        m_log->get(pdal::LogLevel::Debug) << "Labeling object boundaries..." << std::endl;
        labelObjectBoundaries(dtmImage, labelImage, dhBins, dzShort); // Updates labelImage

        // Extend labels for object boundaries..
        m_log->get(pdal::LogLevel::Debug) << "Extending object boundaries..." << std::endl;
        extendObjectBoundaries(dtmImage, labelImage, dhBins, dzShort); // Updates labelImage

        // Group the objects.
        m_log->get(pdal::LogLevel::Debug) << "Grouping objects..." << std::endl;
        std::vector<ObjectType> objects;
        groupObjects(labelImage, dtmImage, objects, maxCount, dzShort); // Updates labelImage, objects
        m_log->get(pdal::LogLevel::Debug) << "Number of objects: " << objects.size() << std::endl;

        // Generate object groups and void fill them in the DEM image.
        m_log->get(pdal::LogLevel::Debug) << "Labeling and removing objects..." << std::endl;
        ...
        m_log->get(pdal::LogLevel::Debug) << "Finishing label image for display..." << std::endl;
        finishLabelImage(labelImage);

        // Update the void image.
        m_log->get(pdal::LogLevel::Debug) << "Updating void image..." << std::endl;
        ...
        m_log->get(pdal::LogLevel::Debug) << "Filling voids with noSmoothing: " << noSmoothing  << std::endl;
    ...
    m_log->get(pdal::LogLevel::Debug) << "Removing spikes... " << std::endl;
    ...
    // Mark all voids.
    m_log->get(pdal::LogLevel::Debug) << "Marking voids in label image after all iterations are complete..." << std::endl;
    for (unsigned int j = 0; j < voidImage.height; j++) {
        for (unsigned int i = 0; i < voidImage.width; i++) {
...
void Shr3dder::classifyNonGround(const OrthoImage<unsigned short> &dsmImage, const OrthoImage<unsigned short> &dtmImage,
    ...
    m_log->get(pdal::LogLevel::Debug) << "Min points for removing small objects: " << minPointCount << std::endl;
    ...
    {
        m_log->get(pdal::LogLevel::Debug) << "Grouping to remove small objects..." << std::endl;
        std::vector<ObjectType> objects;
        ...
        m_log->get(pdal::LogLevel::Debug) << "Number of small objects rejected: " << numRejected  << std::endl;
    }
...
void Shr3dder::fillInsideBuildings(OrthoImage<unsigned char> &classImage) {
    ...
    m_log->get(pdal::LogLevel::Debug) << "Removed " << numFilled << " tree pixels inside building label groups." << std::endl;
}
