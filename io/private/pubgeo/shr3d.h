...
namespace shr3d {
    ...
    public:
        ...
        Shr3dder() : dh_meters(1), dz_meters(1), agl_meters(2),
                ...
                LAS_UNCLASSIFIED(1), LAS_GROUND(2), LAS_TREE(5),
                LAS_BUILDING(6) {}
        ...
        void setLog(pdal::LogPtr l) {m_log = l; }
        void setMap(Json::Value const& m)
        {

            LAS_UNCLASSIFIED = m.get("unclassified", 1).asInt();
            LAS_GROUND = m.get("ground", 2).asInt();
            LAS_TREE = m.get("high", 5).asInt();
            LAS_BUILDING = m.get("building", 6).asInt();
